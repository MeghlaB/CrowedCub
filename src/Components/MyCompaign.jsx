...
export default function MyCampaign() {
  ...
  const getBgClass = () => (theme === "dark" ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-800");
  const getTableClass = () => (theme === "dark" ? "table-auto text-white" : "table-auto text-gray-800");
  ...
  return (
    <div className={` container mx-auto my-28`}>
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className={`${getTableClass()} table`}>
            <thead>
              <tr>
                <th></th>
                <th>Title</th>
            ...
            <tbody>
              {campaigns.map((data, index) => (
                <tr key={data._id}>
                  ...
                  <td>{data.minDonation}</td>
                  <td>
                    <Link to={`/update/${data._id}`} className='btn bg-secondary text-white hover:bg-primary border-none'>
                      Update
                    </Link>
                  </td>
                  <td>
                    <Link onClick={() => handleDelete(data._id)} className='btn bg-red-500 hover:bg-red-400 text-white border-none'>
                      Delete
                    </Link>
                  </td>
                </tr>
